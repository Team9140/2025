 ...
 * as a result).
 * 
 * Modified from team 254's 2023 code.
...
public class SwerveSetpointGenerator {
    ...
    private static final LinearAcceleration kMaxDriveAcceleration = Constants.Drive.MAX_teleop_velocity
            .div(Seconds.of(0.25));
     ...
     */
    public SwerveSetpoint generateSetpoint(final SwerveSetpoint prevSetpoint, ChassisSpeeds desiredState, double dt) {
        SwerveModuleState[] desiredModuleState = mKinematics.toSwerveModuleStates(desiredState);
        // Make sure desiredState respects velocity limits.

        SwerveDriveKinematics.desaturateWheelSpeeds(desiredModuleState, Constants.Drive.MAX_teleop_velocity);
        ...
        if (all_modules_should_flip && !isStopped(prevSetpoint.mChassisSpeeds) && !isStopped(desiredState)) {
